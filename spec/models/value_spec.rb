...
RSpec.describe Value, type: :model do
  ...
  describe 'validations' do
    ...
    it {
      is_expected.to validate_length_of(:value).is_at_least(1).with_message(I18n.t("#{LOCALIZATION_PREFIX}.value.too_short"))
    }
    it {
      is_expected.not_to allow_value('.*+/+/').for(:value).with_message(I18n.t("#{LOCALIZATION_PREFIX}.value.invalid"))
    }
  end
end
