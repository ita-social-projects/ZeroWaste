...
RSpec.describe CalculatorResolver, type: :model do
  ...
  let!(:calculation1) { create(:calculation, value: 'P1 * P2 / P3', type: 'Calculation', label: 'one', kind: 'result', calculator: calculator) }
  let!(:calculation2) { create(:calculation, value: '10 * P4', type: 'Calculation', label: 'one', kind: 'result', calculator: calculator) }
  ...
  let!(:calculation3) { create(:value, value: 'P1 * P2', type: 'Calculation', label: 'four', kind: 'parameter', calculator: calculator) }

  describe "#call" do
    it { expect(CalculatorResolver.call(calculator)).to be_kind_of(Hash) }

    context "when 'field' is 'Calculation'" do
      it { expect(CalculatorResolver.call(calculator)[calculation1]).to match_array([value2, value3, value4]) }
    end
    ...
    context "when 'Calculation' contains nested dependecies" do
      it { expect(CalculatorResolver.call(calculator)[calculation2]).to match_array([value2, value3]) }
    end
  end
end
