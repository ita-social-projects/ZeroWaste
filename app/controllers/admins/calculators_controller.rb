...
class Admins::CalculatorsController < Admins::BaseController
  ...
  def create
    ...
    if @calculator.save
      redirect_to admins_calculators_path, notice: t("notifications.calculator_created")
    else
      render action: "new"
    end
  end
  ...
  def update
    if updater
      redirect_to edit_admins_calculator_path(@calculator), notice: t("notifications.calculator_updated")
    else
      collect_fields_for_form
  ...
  def destroy
    ...
    redirect_to admins_calculators_path, notice: t("notifications.calculator_deleted")
  end
